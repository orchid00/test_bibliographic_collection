...
#' }
#' @export
oadoi_fetch_title <- function(title = NULL, email = NULL) {
  
  title <- "search?query=%22A toolkit for studying Varroa genomics and transcriptomics: Preservation, extraction, and sequencing library preparation&email=paula.martinez@ardc.edu.au"
  u <- httr::modify_url(
    oadoi_baseurl(),
    path = c(oadoi_api_version(), title)
  )
  
  u
  # Call Unpaywall Data API
  resp <- httr::RETRY("GET", u)
